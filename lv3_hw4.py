...
async def start(message: types.Message):
    ...
    inline_btn_4 = InlineKeyboardButton('По приколу', url = 'https://www.youtube.com/watch?v=dQw4w9WgXcQ')
...
async def process_callback_button1(callbak: types.CallbackQuery):
    kb = [
        [KeyboardButton("/peperoni")],
        [KeyboardButton("/four_cheese")],
...
async def start(message: types.Message):
    ...
    await message.answer("Вы заказали пиццу Peperoni")
    cursor = connect.cursor()
    cursor.execute(f"SELECT id_user FROM users WHERE id_user = {message.from_user.id};")
    cursor.execute(f"""INSERT INTO orders VALUES ('peperoni', (SELECT address_longitude + ' ,' + address_latitude + ')' AS address_destination FROM address WHERE id_user = {message.from_user.id}), '{current_datetime1}')""")
    connect.commit()

@dp.message_handler(commands = 'four_cheese')
async def start(message: types.Message):
    current_datetime = datetime.now()
    current_datetime1 = str(current_datetime)
    await message.answer("Вы заказали пиццу four_cheese")
    cursor = connect.cursor()
    cursor.execute(f"SELECT id_user FROM users WHERE id_user = {message.from_user.id};")
    cursor.execute(f"""INSERT INTO orders VALUES ('four_cheese', (SELECT address_longitude + ' ,' + address_latitude + ')' AS address_destination FROM address WHERE id_user = {message.from_user.id}), '{current_datetime1}')""")
    connect.commit()

@dp.message_handler(commands = 'four_season')
async def start(message: types.Message):
    current_datetime = datetime.now()
    current_datetime1 = str(current_datetime)
    await message.answer("Вы заказали пиццу four_season")
    cursor = connect.cursor()
    cursor.execute(f"SELECT id_user FROM users WHERE id_user = {message.from_user.id};")
    cursor.execute(f"""INSERT INTO orders VALUES ('four_season', (SELECT address_longitude + ' ,' + address_latitude + ')' AS address_destination FROM address WHERE id_user = {message.from_user.id}), '{current_datetime1}')""")
    connect.commit()
